...
def get_icon_path():
    ...
    try:
        # Primero intentar con resource_path (para PyInstaller)
    ...
    # Fallback: buscar en PROJECT_ROOT
...
def set_window_icon(window):
    """
    Establece el icono de la ventana de forma multiplataforma.
    
    Args:
        window: La instancia de tk.Tk o ctk.CTk
    """
    ...
    try:
        if platform.system() == "Darwin":  # macOS
            ...
            try:
                ...
                print(f"DEBUG: No se pudo establecer icono en macOS: {e}")
        else:  # Windows y Linux
            window.iconbitmap(icon_path)
    except Exception as e:
        print(f"DEBUG: Error estableciendo icono de ventana: {e}")
...
class MainWindow(TkBase):
    ...
    def __init__(self, launch_target=None, project_root=None, splash_screen=None, app_version="0.0.0"):
        super().__init__()
        ...
        # --- CONFIGURACIÓN DE ICONO MULTIPLATAFORMA ---
        set_window_icon(self)

        # --- CONFIGURACIÓN DE RUTAS DE DATOS SEGÚN PLATAFORMA ---
        if platform.system() == "Windows":
            base_data_path = os.path.join(os.environ.get('LOCALAPPDATA', os.path.expanduser('~')), '')
        elif platform.system() == "Darwin":
            # En Mac: /Users/Nombre/Library/Application Support/DowP
            base_data_path = os.path.expanduser("~/Library/Application Support")
        else:
            # En Linux: /home/Nombre/.config/DowP
            base_data_path = os.path.expanduser("~/.config")

        self.APP_DATA_DIR = os.path.join(base_data_path, 'DowP')

        # 2. Asegurarse de que esa carpeta exista
        try:
            os.makedirs(self.APP_DATA_DIR, exist_ok=True)
        except Exception as e:
            print(f"ERROR: No se pudo crear la carpeta de datos: {e}")
            # Fallback seguro al directorio base de la app
            self.APP_DATA_DIR = self.APP_BASE_PATH

        # 3. Definir las rutas finales usando la carpeta de datos correcta
    ...
    def run_initial_setup(self):
        ...
        def initial_check_task():
            ...
            should_check_online = not (ffmpeg_ok and deno_ok)
        ...
        if not os.path.exists(ffmpeg_path) or not os.path.exists(deno_path):
    ...
    def on_update_check_complete(self, update_info):
        ...
        def _ui_task():
            if update_info.get("update_available"):
                ...
                self.single_tab.app_status_label.configure(text=f"DowP Lite v{self.APP_VERSION} - Error al verificar", text_color="orange")
                self.single_tab.update_app_button.configure(text="Reintentar", state="normal", fg_color="gray")
            else:
                self.single_tab.app_status_label.configure(text=f"DowP Lite v{self.APP_VERSION} - Estás al día ✅")
